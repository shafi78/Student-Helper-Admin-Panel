...
const ManageDsaContest = () => {
    ...
    const getQuestions = async() => {
        const data = await getDocs(collection(db, "DSAContest"));
        setQuestions(data.docs.map((doc) => ({...doc.data(), id: doc.id})))
    }

    const handleDelete = async(id) => {
        const QnaDoc = doc(db,"DSAContest",id);
...
export default ManageDsaContest